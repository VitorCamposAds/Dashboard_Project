...
from .models import Produto, Vendas, Vendedor, Despesa
...
def renderiza_despesas(request):
    ...
    for mes_numero, nome_mes in enumerate(meses, start=1):
        despesas_mes = x.filter(data__year=ano_atual, data__month=mes_numero)
        valor_total_despesas = despesas_mes.aggregate(Sum('valor'))['valor__sum'] or 0

        labels.append(nome_mes)
        data.append(valor_total_despesas)
